data {
  ...
  vector[SY] end_trump;                    // dummy with 1 = Rep. cand. endosed by trump
  vector[SY] inc_rep;                      // dummy with 1 = Rep. incumbent
...
transformed parameters {
  ...
  // Mean model
    ...
    beta4 = mu_beta4 + sig_beta4 * beta4_sc;
...
model {
  ...
  // Mean model
    ...
    mu_beta4 ~ normal(0,0.2);
    sig_beta4 ~ normal(0,0.2) T[0,]; // half normal due to bounds
  ...
  for(i in 1:N){
    logit_p[i] = logit_vote[sy_id[i]] + 
      alpha[sy_id[i]] + 
      beta1 * end_trump[sy_id[i]] + 
      beta2[sy_id[i]] * t[i] + 
      beta3 * inc_rep[sy_id[i]] + 
      beta4 * inc_rep[sy_id[i]] * end_trump[sy_id[i]]; 
